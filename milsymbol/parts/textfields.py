...
def str_width(string, font_size):
    ...
    char_widths = {
      " ": 9, "!": 10, '"': 15, "#": 17, "$": 17, "%": 27, "&": 22, "'": 8,
      "(": 10, ")": 10, "*": 12, "+": 18, ",": 9, "-": 10, ".": 9, "/": 9,
      "0": 17, "1": 17, "2": 17, "3": 17, "4": 17, "5": 17, "6": 17, "7": 17, "8": 17, "9": 17,
      ":": 10, ";": 10, "<": 18, "=": 18, ">": 18, "?": 19, "@": 30,
      "A": 22, "B": 22, "C": 22, "D": 22, "E": 21, "F": 19, "G": 24, "H": 22, "I": 9, "J": 17, "K": 22,
      "L": 19, "M": 25, "N": 22, "O": 24, "P": 21, "Q": 24, "R": 22, "S": 21, "T": 19, "U": 22, "V": 21,
      "W": 29, "X": 21, "Y": 21, "Z": 19,
      "[": 10, "]": 10, "^": 18, "_": 17, "`": 10,
      "a": 17, "b": 19, "c": 17, "d": 19, "e": 17, "f": 10, "g": 19, "h": 19, "i": 9, "j": 9, "k": 17,
      "l": 9, "m": 27, "n": 19, "o": 19, "p": 19, "q": 19, "r": 12, "s": 17, "t": 10, "u": 19, "v": 17,
      "w": 24, "x": 17, "y": 17, "z": 15,
      "{": 12, "|": 9, "}": 12, "~": 18
    }
...
def textfields(symbol):
    ...
    font_color = style.infoColor if style.infoColor else getattr(colors["iconColor"], affiliation, "black")
    ...
    g_strings = {k: "" for k in ["L1", "L2", "L3", "L4", "L5", "R1", "R2", "R3", "R4", "R5"]}
    ...
    if is_air:
        ...
        l2_parts = []
        if options.altitudeDepth: l2_parts.append(options.altitudeDepth)
        if options.location: l2_parts.append(options.location)
        ...
        # R5 Eval rating etc.
        r5_parts = []
        if options.evaluationRating: r5_parts.append(options.evaluationRating)
        if options.combatEffectiveness: r5_parts.append(options.combatEffectiveness)
        if options.signatureEquipment: r5_parts.append(options.signatureEquipment)
        if options.hostile: r5_parts.append(options.hostile)
        if options.iffSif: r5_parts.append(options.iffSif)
        g_strings["R5"] = "/".join(r5_parts)
        
        # L3 Type/Platform
        l3_parts = []
        if options.type: l3_parts.append(options.type)
        if options.platformType: l3_parts.append(options.platformType)
        if options.equipmentTeardownTime: l3_parts.append(options.equipmentTeardownTime)
        ...
        # R3
        r3_parts = []
        if options.additionalInformation: r3_parts.append(options.additionalInformation)
        if options.commonIdentifier: r3_parts.append(options.commonIdentifier)
    ...
    if g_strings["L1"]:
        draw_array.append({
            "type": "text", "text": g_strings["L1"],
            "x": bbox.x1 - space_text_icon, "y": 100 - 1.5 * font_size,
            "textanchor": "end", "fontsize": font_size, "fontfamily": font_family,
            "fill": font_color, "stroke": False
        })
    # L2
    if g_strings["L2"]:
        draw_array.append({
            "type": "text", "text": g_strings["L2"],
            "x": bbox.x1 - space_text_icon, "y": 100 - 0.5 * font_size,
            "textanchor": "end", "fontsize": font_size, "fontfamily": font_family,
            "fill": font_color, "stroke": False
        })
    # L3
    ...
    if g_strings["L5"]:
        draw_array.append({
            "type": "text", "text": g_strings["L5"],
            "x": bbox.x1 - space_text_icon, "y": 100 + 2.5 * font_size,
            "textanchor": "end", "fontsize": font_size, "fontfamily": font_family,
            "fill": font_color, "stroke": False
        })
    ...
    if g_strings["R1"]:
        draw_array.append({
            "type": "text", "text": g_strings["R1"],
            "x": bbox.x2 + space_text_icon, "y": 100 - 1.5 * font_size,
            "textanchor": "start", "fontsize": font_size, "fontfamily": font_family,
            "fill": font_color, "stroke": False
        })
    # R2
    if g_strings["R2"]:
        draw_array.append({
            "type": "text", "text": g_strings["R2"],
            "x": bbox.x2 + space_text_icon, "y": 100 - 0.5 * font_size,
            "textanchor": "start", "fontsize": font_size, "fontfamily": font_family,
            "fill": font_color, "stroke": False
        })
    # R3
    ...
    if g_strings["R5"]:
        draw_array.append({
            "type": "text", "text": g_strings["R5"],
            "x": bbox.x2 + space_text_icon, "y": 100 + 2.5 * font_size,
            "textanchor": "start", "fontsize": font_size, "fontfamily": font_family,
            "fill": font_color, "stroke": False
        })
    ...
    if options.quantity:
        draw_array.append({
            "type": "text", "text": options.quantity,
            "x": 100, "y": bbox.y1 - 10,
            "textanchor": "middle", "fontsize": font_size, "fontfamily": font_family,
            "fill": font_color, "stroke": False
        })
    ...
    if options.headquartersElement:
        draw_array.append({
            "type": "text", "text": options.headquartersElement,
            "x": 100, "y": bbox.y2 + 35,
            "textanchor": "middle", "fontsize": 35, "fontfamily": font_family,
            "fontweight": "bold", "fill": font_color, "stroke": False
        })
