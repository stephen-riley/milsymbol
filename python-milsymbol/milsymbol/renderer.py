...
class Renderer:
    ...
    def _process_instruction(self, instruction, parent):
        ...
        if itype == "path":
            ...
            g = ET.SubElement(
                parent, "g", {"transform": f"scale({instruction.get('factor', 1)})"}
            )
            for sub in instruction.get("draw", []):
                self._process_instruction(sub, g)

        elif itype == "rotate":
            degree = instruction.get("degree", 0)
            x = instruction.get("x", 0)
            y = instruction.get("y", 0)
            g = ET.SubElement(parent, "g", {"transform": f"rotate({degree},{x},{y})"})
            for sub in instruction.get("draw", []):
                self._process_instruction(sub, g)
    ...
    def _get_color(self, color, default=None):
        ...
        if isinstance(color, dict):
            return "none"  # Should not happen if resolved correctly
        return str(color)
